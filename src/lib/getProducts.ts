...
export async function getProducts(): Promise<Product[]> {
  const rows: any = await query(`
    ...
    FROM anto_productos p
    LEFT JOIN anto_marcas m ON p.id_marca = m.id_marca
    WHERE p.oculto = 0
      AND p.obsoleto = 0
      AND p.stock > 0
  ...
  return rows.map((row: any): Product => {
    ...
    // Si la imagen existe en la DB, construimos la ruta real hacia el servidor de Jonathan.
    // Si no tiene imagen, ponemos un fallback genérico.
    ...
    return {
      ...
      description: row.descripcion ? row.descripcion.replace(/<[^>]*>?/gm, '') : 'Sin descripción detallada.', // Quitamos etiquetas HTML si las hubiera
      image: imagePath,
      features: [],
      compatible: [],
      badge: row.masvendido === 1 ? 'Destacado' : undefined,
    };
  });
}
...
export async function getProductById(id: string): Promise<Product | null> {
  ...
  return {
    ...
    price,
    priceFormatted: price > 0 ? formatPrice(price) : 'Consultar',
    description: row.descripcion ? row.descripcion.replace(/<[^>]*>?/gm, '') : 'Sin descripción detallada.',
    image: imagePath,
    features: ['Consulta compatibilidad vía WhatsApp', 'Instalación garantizada en nuestras sucursales', 'Soporte técnico directo'],
    compatible: ['Apto para múltiples vehículos, consulta con un técnico.'],
    badge: row.masvendido === 1 ? 'Destacado' : undefined,
  };
}
